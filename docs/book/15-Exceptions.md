...
因为异常处理是Java报告错误的唯一官方方式，并被编译器强制要求，如果不学习异常处理的话，Java学习不会很深入。本章将教你如何编写正确的异常处理程序，以及当方法出问题的时候，如何产生自定义的异常。
...
异常允许你将做的每件事都当作一个事务来考虑，而异常守护着这些事务：“......事务的基本保障是我们所需的分布式计算中的异常处理。事务相当于计算机中的合同法。如果任何事出现了错误，我们只需要扔掉整个计算。”你也可以将异常看作一种内建的“恢复”（undo）系统，因为（在细心使用时）你在程序中可以有各种恢复点。如果程序的一个部分失败了，异常将“恢复”到一个已知的稳定点上。
