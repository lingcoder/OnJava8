...
我们可以将编程的侧重领域区分为研发和应用。应用程序员调用前者构建的基础工具类来快速开发。研发程序员构建一个类，该类仅向应用程序员公开必要的内容，并保护其他所有内容。隐藏技术实现的细节可以有效避免错误的使用和改变，因此隐藏实现可以减少程序错误。双方有着各自的工作侧重点，彼此尊重。当您创建库时，您与客户端程序员建立了关系，客户端程序员也是程序员，但是通过使用您的库来组装应用程序，可能是为了构建更大的库。如果研发提供的工具暴露了内部所有内容，那么有些规则就不好被遵守。你无法保证应用程序员不去改变它。即使你不喜欢应用程序员直接操作你的某些方法，但是只要有没有访问控制，你就没办法阻止别人怎么做。

因此，访问控制的第一个原因是让为了让应用程序员不要触摸他们不应该触摸的部分。（请注意，这也是一个哲学决策。部分编程语言认为如果程序员有需要，则应该让他们访问细节部分。）；第二个原因是使库设计者不影响应用程序员使用库情况下更新完善类。例如，您可以以简单的方式实现特定的类以简化开发，然后发现您必须重写它以使其运行得更快。如果界面和实现明确分开并受到保护，您可以轻松完成此任务。Java 有三个显式关键字来设置类中的边界：public，private和protected。这些访问说明符确定谁可以使用后面的定义。public意味着每个人都可以使用该元素。private意味着没有人可以访问该元素，除了您，该类型的创建者，该类型的内部方法。私人是你和客户程序员之间的砖墙。任何试图访问私有成员的人都会收到编译时错误。受保护的行为类似于私有，但是继承类可以访问受保护的成员，但不能访问私有成员。很快就会引入继承。Java还具有“默认”访问权限，如果您不使用上述说明符之一，它将发挥作用。这通常称为包访问，因为类可以访问同一包（库组件）中其他类的成员，但在包外，这些相同的成员看起来是私有的。
