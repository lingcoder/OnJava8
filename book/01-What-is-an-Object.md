...
我们可以将这些问题一一分解，抽象成一组服务。软件设计的基本原则是高内聚：每个组件的内部作用明确，功能紧密相关。然而经常有人将太多功能塞进一个对象中。例如：在支票打印模块中，你需要设计一个可以同时读取文本格式又能正确识别不同打印机型号的对象。正确的做法是提供三个或更多对象：一个对象检查所有排版布局的目录；一个或一组可以识别不同打印机型号的对象展示通用的打印界面；第三个对象组合上述两个服务来完成任务。这样，每个对象都提供了一组紧密的服务。在良好的面向对象设计中，每个对象功能单一且高效。这样的程序设计可以提高我们代码的复用性，同时也方便别人阅读和理解我们的代码。只有让人知道你提供什么服务，别人才更好地将其应用到其他模块或程序中。


## 封装


我们可以将编程的侧重领域区分为类创建者和客户端程序员。客户端程序员调用类创建者构建的基础工具类来快速开发。类创建者构建一个类，该类仅向客户端程序员公开必要的内容，并保护其他所有内容。隐藏技术实现的细节，避免程序被错误的使用和改变，减少程序错误。双方有着各自的工作侧重点，彼此尊重。当创建者创建库时，与客户端程序员建立了关系，客户端程序员通过使用创建者的库来组装应用程序或者构建更大的库。如果创建者提供的工具库暴露了工具本身的所有内容，那么有些规则就不好被遵守。你无法保证客户端程序员不去改变它。即使你不喜欢客户端程序员直接操作你的工具中的某些方法，但是只要没有访问控制，你就没办法阻止这种做法。

因此，使用访问控制的原因：

1. 让客户端程序员不要触摸他们不应该触摸的部分。（请注意，这也是一个哲学决策。部分编程语言认为如果程序员有需要，则应该让他们访问细节部分。）；

2. 使库创建者在不影响客户端程序员使用库情况下更新完善工具库。例如，你以粗陋的方式开发了开发额一个类来简化任务，后来你发现必须重写它以使其运行得更快。如果应用程序接口和实现部分明确分开并受到保护，您可以轻松完成改造。

Java 有三个显式关键字来设置类中的访问权限：`public`（公开），`private`(私有)和`protected`（受保护）。这些修饰符确定谁可以使用后面的定义。`public` 表示任何人都可以访问和使用该元素；`private` 说明除了类本身，外界无法直接访问该元素和内部方法。`private`是你和客户程序员之间的砖墙。任何试图访问私有成员的人都会收到编译时错误。`protected`的行为类似于`private`，区别是继承类可以访问`protected`的成员，但不能访问`private`成员。下一节就会引入继承的概念。Java 还具有`default`（默认）访问权限，如果你不使用前面的三者，默认就是`default`访问权限。`default`被称之为包访问，因为类可以访问同一包（库组件）中其他类的成员。
